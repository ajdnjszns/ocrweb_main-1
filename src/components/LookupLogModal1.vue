          ...
          if(this.filename){
            ...
            for(let i = 0; i < this.pagecnt; i++){
              ...
              this.textfile.push(this.filepath + '/' + this.filename + '_cocr_' + this.lpad(i+1, 4, '0') + '_CODE1OCR.txt')
                ...
                console.log("######## JPG ########",this.filepath + this.filepath.substring(this.filepath.lastIndexOf("/"), this.filepath.length))
                ...
                // this.jsonfile.push(this.filepath + '/' + this.filename.substring(this.filename.lastIndexOf("/"), this.filename.lastIndexOf(".")) + "_CODE1DC.json")
    ...
    methods: {
      ...
      imageSelect(idx){
        if(idx=='first'){
          ...
          // this.getJson(this.jsonfile[this.imgArr.length-1]);
        } else if (idx != '') {
      ...
      getText(url){
        console.log('get encodeURIComponent Text =>',url)
        axios
          .get(url, {},
          {
          ...
          .then((res) => {
            ...
            for(let i = 0; i < this.splitedStr.length; i++){
              if(this.splitedStr[i].includes('id_card_type')){
                ...
                this.splitedStr[i] = this.splitedStr[i].replace('biz_lic_type', '사업자등록종별')
                ...
                this.splitedStr[i] = this.splitedStr[i].replace('total_amount', '총 금액')
              }else if(this.splitedStr[i].includes('idcard_type_jumin')){
                this.splitedStr[i] = this.splitedStr[i].replace('idcard_type_jumin', '주민등록증')
              }
  
              if(this.splitedStr[i].includes('idcard_type_driver')){
                this.splitedStr[i] = this.splitedStr[i].replace('idcard_type_driver', '운전면허증')
              }
              ...
              if(this.splitedStr[i].includes('corporation')){
                this.splitedStr[i] = this.splitedStr[i].replace('corporation', '법인')
              }
              if(this.splitedStr[i].includes('personal')){
                this.splitedStr[i] = this.splitedStr[i].replace('personal', '개인')
              }
              if(this.splitedStr[i].includes('unique')){
                this.splitedStr[i] = this.splitedStr[i].replace('corporation', '고유번호증')
              }
